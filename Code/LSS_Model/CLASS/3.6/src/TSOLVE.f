...
C
      DO 50 I=IL1,IL2
          IF(FI(I).GT.0.)                                          THEN
              ...
              ! TRTOP: the amount of radiation transmitted into surface.  
...
C  
  100 CONTINUE
C
      ! NIT : There are still locations at the beginnig  of the current iteration step
	  ! for which the surface temperature has not yet been found 
	  ! NUMIT : There are still locations at the beginnig  of the current iteration step
	  ! for which the surface temperature has not yet been found 
	  NUMIT=0
      NIT=0
      DO 150 I=IL1,IL2
          IF(FI(I).GT.0. .AND. ITER(I).EQ.1)                       THEN    
              ...
              ELSE                
                  ...
                  B(I)=7.66     
              ENDIF                       
              ! the saturation mixing ratio (pp . 107)
...
C
     ...
     1                           TZERO(I).GT.373.16))               THEN 
			  ...
			  ! Added by Ala Bahrami to resolve the problematic of model crashing
			  IBAD=I 
			  write(*,*)'IBAD',I
			  !TZERO(I) = 273.16
          ...
          ENDIF
			  ...
			  ! IF(FI(I).GT.0. .AND. (TZERO(I).GT.173.16 .OR. TZERO(I).LT.373.16)) THEN                 
			   	! Problematic memeber ensemble memeber 12, date 153
				! if (I.EQ.2588) then
						! write (*,*) 'ISNOW' , ISNOW 
						! write (*,*) 'TZERO(2588) ', TZERO(2588)
				! endif 		
