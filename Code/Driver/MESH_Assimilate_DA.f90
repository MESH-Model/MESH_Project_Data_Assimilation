...
!		   30/10/2017 - Reading perturbed GRACE observations  	 
...
!					  - Adding the capability of running the program in the assimilation mode (second run)
!		   19/01/2018 - 
...
!					  -	
!		   18/04/2018 - 1)Modifying the code to update ic%start%jday when OLDA code runs for the 
...
!					  - 2) Adding the variable month_n to avoid allocating GR_Pert varible manually 
!
!		  24/07/2018  -
...
!todo: clean-up.
	...
	integer JDAY_IND_MET1, ISTEP_START1 , nmy1, nhy1, nrs1, nrs1_mon, Jday_IND21, Jday_IND31 
...
!so, make them local variables inside each read subroutine.
	...
	iun = 960
	open (iun, file = 'Inputs/Assimilation_parameters_perturbation_da.ini')
	...
	! HT. Fortan cannot transpose of array of rank 1 
	...
	! Allocate model predicted TWS vriables and analysis increments
	...
	! Allocate normalized Innovaiton
	if (allocated(NI)) deallocate(NI)
	allocate(NI(month_n , N_ens))
	...
	stg_accum(1,:) = 0
	!----------------------------------------------------------------------
	! this modification is activated when the code is crashed in the analysis mode 
	! note : don't foreget to commenet it 
	! it_counter = 2
	! assim_mode = .true. 
	
	! iun = 961
	! open (iun, file = 'Outputs/AI_20_new.ini')
	
	! do i = 1, 13*NML
					 ! read(iun,*) (AI(i,j) , j = 1, N_ens)
	! end do 
	...
	!------------------------------------------------------------------------
	! open writing normalized Innovaiton matrix 
	open(80, file = 'Outputs/NI.dat', action = 'write')
	
	
	
	! todo : check to remove 400 or 500
400 continue 
...
do tt = 1 , N_ens
	...
	!            print *, cm%dat(8)%alpha
		...
		!nrs1_mon = nint(nrs1/(48 * 30.5))
		
		write(*,*) 'numbers of skipped forcepert recoreds',nrs1
		!write(*,*) 'numbers of skipped months',nrs1_mon
...
2835    	format(/1x, 'STARTING MESH')
		...
		! Note : if the progrma is in the assimilation mode this variable should be changed!
		! todo : this section should be revised. To see whether it is needed
	...
	!-        if (inp > 1 .and. ic%ts_daily == MPIUSEBARRIER) call MPI_Barrier(MPI_COMM_WORLD, ierr)
					...
					if(ndy == 1) then
						if (ic%now%hour .eq. 0 .and. ic%now%mins .eq. 0) then
							...
							X( 12*NML+1 : 13*NML, tt) = stas%sl%thic(:,4)
								...
								! !>> calculate the KG
								! ! Calculate Kalman Gain Matrix ,the X_ave and 
								if (tt .eq. N_ens) then
									...
									!! Calculate X5
									X5 = 1./((matmul(H , X4) + GR_er**2.))
									
									X6 = sqrt(matmul(H , X4) + GR_er**2.)
			...
			!!if (ipid == 0 ) then 
				if (assim_mode .and. ic%ts_daily == 48) then 
								  ...
								  ! commented this part to avoid code crashing
								  !stas%cnpy%sncan   = stas%cnpy%sncan   + AI( 1:NML, tt)
								  !stas%cnpy%rcan    = stas%cnpy%rcan    + AI( 1*NML+1 : 2*NML, tt)
								  								  
								  !!stas%sno%sno      = stas%sno%sno      + AI( 2*NML+1 : 3*NML, tt)
								  do i = 1 , NML
										
										! Avoid updating sno and wsno for some problematic elements
										! which create crashing problem in CLASSZ
										
										if (stas%cnpy%sncan(i) .le. 0) then 
											stas%cnpy%sncan(i) = 0.
										else 
											stas%cnpy%sncan(i)      = stas%cnpy%sncan(i)      + AI(i, tt)
											if ( stas%cnpy%sncan(i) .lt. 0) then 
												stas%cnpy%sncan(i) = stas%cnpy%sncan(i) - AI(i, tt)
											end if
										end if
										
										
										if (stas%cnpy%rcan(i) .le. 0) then 
											stas%cnpy%rcan(i) = 0.
										else 
											stas%cnpy%rcan(i)      = stas%cnpy%rcan(i)      + AI(NML+i, tt)
											if ( stas%cnpy%rcan(i) .lt. 0) then 
												stas%cnpy%rcan(i) = stas%cnpy%rcan(i) - AI(NML+i, tt)
											end if
										end if
										
										
										if (stas%sno%sno(i) .le. 0.) then 
											stas%sno%sno(i) = 0.
										else 
											stas%sno%sno(i)      = stas%sno%sno(i)      + AI( 2*NML+i, tt)
											if (stas%sno%sno(i) .lt. 0.1) then 
												stas%sno%sno(i) = stas%sno%sno(i) - AI( 2*NML+i, tt)
											end if 
										end if 
										
										 
										if (stas%sno%wsno(i) .le. 0) then 
											stas%sno%wsno(i) = 0.
										else 
											stas%sno%wsno(i)      = stas%sno%wsno(i)      + AI( 3*NML+i, tt)
											if (stas%sno%wsno(i) .lt. 0) then 
												stas%sno%wsno(i) = stas%sno%wsno(i) - AI( 3*NML+i, tt)
											end if
										end if 
								   ...
								   ! todo : set the condition to avoid code crashing   			
			...
			! see : save_basin_output
			
			! !Accumulate daily Storage Value 
			if (ipid == 0) then 
			...
			! ! Calculate the ensemble innovation vector 
			if(ipid == 0) then 
				if (ic%ts_daily == 48 .and. .not. assim_mode) then 
					...
					if(ndy == 1) then
					    ...
					    !save the ouput in .ini file  
						open (11, file = 'Outputs/Innovation_20_Monthly_new.ini')
						write(11,*) D (1 , tt)
						
						! if (.not. isnan(D (1 , tt))) then 
								! na_data = .false. 
						! else 
								! na_data = .true. 
						! end if

						if (GR_Pert(tt , month_counter) .ne. -999.) then 
								na_data = .false.
						else 
								na_data = .true. 
						end if 
			...
			!> ------------------------------------------------------------
			! Note : If ipid is assigned it correctly calculate the AI, but the code is hanged 
			! and it does not give the results of the analysis mode 
			
			if (ipid == 0) then 
			
				if (tt .eq. N_ens .and. it_counter .eq. 2) then
						...
						if (ic%ts_daily == 48) then 
							if (ndy == 1) then
								...
								! verifying if the perturbed GRACE observations have NaN values  
								if (.not. na_data) then 
											
										AI = matmul(KG, D)
										! calculate the fraction of analysis increment for each day of the month 
										AI =  AI / ndy
										
										!writing the Analysis increment  
										open (13, file = 'Outputs/AI_20_new.ini')
										do i = 1, 13*NML
														 write(13,*) (AI(i,j) , j = 1, N_ens)
										end do 
										close(13)
										! close the Innovation Matrix 
										close(11)
										
										! Calculating Normalized Innovation
										NI(month_counter,:) = D(1 , :)/X6(1,1) 
										
										! writing the NI matrix 
										write(80,*) (NI(month_counter,j) , j = 1, N_ens)
										
										! closing writing of file
										if (month_counter.eq. month_n) then
												close(80)
										end if 
												
								else
										AI = 0 
										write (*,*) 'GRACE observations were not available at this month'  
										close(13)
										! close the Innovation Matrix 
										close(11)
										
										! Calculating Normalized Innovation
										NI(month_counter,:) = -999 
...
9002    	format(/1x, 'ERROR IN READING r2c_output.txt FILE.', &
		...
		! In this section programs switches from the forecast mode to analysis mode
		! after finishing the last ensemble member of the forecast. 	
		if (assim_mode .and. it_counter .eq. 2 .and. .not. na_data) then 	
				goto 400
		end if 
		...
		! activated when the analysis mode is activated 
		if (assim_mode .and. .not. na_data) then
		...
		! activated when the GRACE observations were not available 
		if (na_data) then 	
			write(strens , '(I5)') , tt
			fl_ens = 'cp -R'//' resume_ens/forecast/'// 'ens'// trim(adjustl(strens))// ' resume_ens/analysis'
			call system(trim(fl_ens))
		end if 
		
		
		if (assim_mode .and. tt .eq. N_ens) then 
					it_counter    =  1
					assim_mode    = .false.
